#!/usr/bin/env python3
"""Download edtdoc (1C:EDT documentation) from ITS (authenticated) and convert to clean text."""
...
DOCS_DIR = os.path.join(REPO_ROOT, "docs", "edtdoc")
BASE = "https://its.1c.ru"
URLS_FILE = os.path.join(SCRIPT_DIR, "edtdoc_urls.txt")
...
def main():
    if not os.path.exists(SESSION):
        print(f"ERROR: session file not found: {SESSION}")
        print("Export browser cookies for its.1c.ru into Netscape format and save as _session.txt")
        return
    ...
    for i, url_path in enumerate(urls, 1):
        ...
        try:
            ...
            src_match = re.search(r'"src":"(/db/content/[^"]*\.html?)"', bus_text)
            if not src_match:
